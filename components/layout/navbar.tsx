  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--color-border)] bg-[var(--color-bg)]/80 backdrop-blur-xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`px-3 py-2 text-sm rounded-[var(--radius-md)] transition-colors duration-[var(--motion-fast)] ${
                  isActive
        ...
        {/* Mobile menu button */}
        <button
          className="md:hidden p-2 text-[var(--color-text-muted)] hover:text-[var(--color-text)] cursor-pointer"
