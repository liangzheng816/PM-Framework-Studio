...
Problem Tree is a **problem framing** framework used to separate the real problem from noise, symptoms, assumptions, and premature solution ideas. The framework is best described as: **Breaks a problem into causes, sub-causes, and effects. Best when stakeholders keep mixing root causes with downstream symptoms.**. In practical product work, its value is that it gives teams a repeatable way to move from ambiguity to a clearer decision, artifact, or next experiment without pretending there is more certainty than the evidence actually supports.
